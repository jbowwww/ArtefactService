...
namespace Artefacts.Service
{
	...
	public class RepositoryClientProxy : IRepository, IQueryProvider, IOrderedQueryable<Artefact>, IDisposable
	{
		...
		public void Init()
		{
			...
			{ 
				...
				Console.WriteLine ("\tBinding: {0}\n\tAddress: {1}\n", Binding.ToString (), Address.ToString ());
...
//				_clientId = Channel.Connect(Host.Current);
				...
				Console.WriteLine ("Artefacts.Host.Current: {0}\n", Host.Current.ToString ());
...
//			return (IQueryable)mi.Invoke(this, new object[] { expression });
		...
		IQueryable<TElement> IQueryProvider.CreateQuery<TElement>(Expression expression)// where TElement : Artefact
		{
			...
			object parsedExpressionId = parsedExpression.Id();
			IQueryable query;
			if (QueryCache.TryGetValue(parsedExpressionId, out query))
				return (IQueryable<TElement>)query;
			QueryCache[parsedExpressionId] = (IQueryable)typeof(Queryable<>).MakeGenericType(typeof(TElement))
			...
			return (IQueryable<TElement>)QueryCache[parsedExpressionId];
		}
